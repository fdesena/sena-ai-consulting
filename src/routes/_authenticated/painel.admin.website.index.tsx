...
function TrafficPage() {
  ...
  const ctaSources = useMemo(() => {
    const m = new Map<string, number>();
    rows
      .filter((r) => r.event === "click_diagnostico_cta")
      .forEach((r) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="inline-flex rounded-full bg-card border border-border p-1">
          {[7, 30, 90].map((d) => (
            <button
              key={d}
              onClick={() => setDays(d)}
              className={`px-3 py-1.5 text-xs rounded-full ${days === d ? "bg-bronze text-black font-semibold" : "text-muted-foreground"}`}
            >
              {d}d
            </button>
          ))}
        ...
        <button
          onClick={exportCsv}
          className="inline-flex items-center gap-2 rounded-full border border-border px-4 py-2 text-xs hover:border-bronze"
        >
      ...
      <div className="grid sm:grid-cols-4 gap-4">
        ...
        <Kpi
          label="Cliques no diagnóstico"
          value={String(stats.ctaClicks)}
      ...
      <div className="rounded-2xl border border-border bg-card p-5">
        ...
        <p className="text-sm text-foreground">
          De cada 100 visitantes,{" "}
          <span className="text-bronze font-semibold">{stats.conversionRate}</span> clicaram em
